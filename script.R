...
df <- read_csv("R-Ladies 2017 Polyanna (Respuestas) - Respuestas de formulario 1.csv") %>% 
  rename(email = `Dirección de correo electrónico`) %>%
  rename(name = Name) 

set.seed(525) # Set seed to reproduce matching in case not every mail gets to its destination
dat <- df %>%
  select(name) %>%
...
parcel <- parcel_create(dat, sender_name = "Sender", sender_email = "email@gmail.com",
